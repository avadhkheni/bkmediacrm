...
import { Monitor, Download, Layers, Box } from 'lucide-react';
...
export default function LedReport() {
  ...
  const handleExport = () => {
    if (!data) return;
    const rows = [['Metric','Value'],['Total Entries',data.totalEntries],['Total Cabinets',data.totalCabinets],['Total Boxes',data.totalBoxes],['Active Allocations',data.activeAllocations]];
    const csv = "data:text/csv;charset=utf-8," + rows.map(r=>r.join(",")).join("\n");
    const link = document.createElement("a"); link.href = encodeURI(csv); link.download = `LED_Report_${new Date().toISOString().split('T')[0]}.csv`; document.body.appendChild(link); link.click(); document.body.removeChild(link);
  };

  if (loading) return <div className="py-12 text-center text-slate-500">Loading LED analytics...</div>;
