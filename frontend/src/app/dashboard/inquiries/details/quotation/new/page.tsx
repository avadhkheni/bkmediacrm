...
function NewQuotationContent() {
  ...
  useEffect(() => {
    ...
    const fetchInquiry = async () => {
      try {
        const [inqRes, ledRes, videoRes, ratesRes] = await Promise.all([
            ...
            {fields.map((field, index) => (
              <div key={field.id} className="grid grid-cols-1 md:grid-cols-12 gap-4 p-4 border border-slate-100 dark:border-slate-700 rounded-xl relative group">
                <div className="md:col-span-3">
                  ...
                  <input {...register(`items.${index}.placeName`, { required: 'Place name is required' })} className={`w-full text-sm p-2 rounded-md border ${errors.items?.[index]?.placeName ? 'border-red-400 bg-red-50' : 'border-slate-200 bg-white'} dark:bg-slate-700 dark:border-slate-600 text-slate-900 dark:text-white`} placeholder="e.g. Main Venue, Stage" />
                </div>
                ...
                {inquiry.department === 'VIDEO' ? (
                  ...
                  <>
                    ...
                    <div className="md:col-span-1">
                      ...
                      <input type="number" min="0" {...register(`items.${index}.widthFt`, { required: 'Required', min: { value: 0, message: 'Min 0' } })} className={`w-full text-sm p-2 rounded-md border ${errors.items?.[index]?.widthFt ? 'border-red-400 bg-red-50' : 'border-slate-200 bg-white'} dark:bg-slate-700 dark:border-slate-600 text-slate-900 dark:text-white`} />
                    </div>
                    <div className="md:col-span-1">
                      <label className="block text-xs text-slate-500 mb-1">H (ft)</label>
                      <input type="number" min="0" {...register(`items.${index}.heightFt`, { required: 'Required', min: { value: 0, message: 'Min 0' } })} className={`w-full text-sm p-2 rounded-md border ${errors.items?.[index]?.heightFt ? 'border-red-400 bg-red-50' : 'border-slate-200 bg-white'} dark:bg-slate-700 dark:border-slate-600 text-slate-900 dark:text-white`} />
                    </div>
                    <div className="md:col-span-1">
                    ...
                    <div className="md:col-span-1">
                      ...
                      <input type="number" min="0" {...register(`items.${index}.ratePerSqft`, { required: 'Required', min: { value: 0, message: 'Min 0' } })} className={`w-full text-sm p-2 rounded-md border ${errors.items?.[index]?.ratePerSqft ? 'border-red-400 bg-red-50' : 'border-slate-200 bg-white'} dark:bg-slate-700 dark:border-slate-600 text-slate-900 dark:text-white`} />
                    </div>
                  </>
                )}
