...
export default function InvoiceDetailPage() {
  ...
  const fetchInvoiceDetails = async () => {
    try {
      ...
      if (data.balanceAmount !== null && data.balanceAmount !== undefined) {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
              ...
              <p className="text-2xl font-black text-slate-900 dark:text-white flex items-center">
                <IndianRupee className="w-5 h-5" /> {(invoice.grossTotal || 0).toLocaleString()}
              </p>
            </div>
            <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
              ...
              <p className="text-2xl font-black text-green-600 dark:text-green-400 flex items-center">
                <IndianRupee className="w-5 h-5" /> {(Number(invoice.grossTotal || 0) - Number(invoice.balanceAmount || 0)).toLocaleString()}
              </p>
            </div>
            <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
              ...
              <p className="text-2xl font-black text-amber-600 dark:text-amber-400 flex items-center">
                <IndianRupee className="w-5 h-5" /> {(invoice.balanceAmount || 0).toLocaleString()}
              </p>
            </div>
          </div>
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                    {invoice.payments.map((p) => (
                      <tr key={p.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                        ...
                        <td className="py-4 px-6 text-sm font-bold text-green-600 text-right">₹{(p.amount || 0).toLocaleString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
